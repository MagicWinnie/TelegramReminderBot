...
Вы можете **2** раза запросить промежуточное ревью. Оно не повлияет на итоговую оценку. Дедлайны запроса ревью указаны в формах на платформе EDU. Имейте в виду, что ревью не подразумевает выставление промежуточной оценки, мы оставим лишь комментарии в MR.
...
1. Если вы хотите отправить работу на предварительное ревью, создайте новую ветку (`review1`, `review2`) от последнего коммита в `dev`, который вы хотите включить в ревью, откройте MR ⚠️**в свой репозиторий**⚠️ и отправьте ссылку в нужную форму на EDU. Мержить после ревью ничего не нужно! Вашу работу проверит прикрепленный к вам ассистент, который проверял домашние работы.
